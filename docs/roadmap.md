...
- [ ] Add experiment metadata schema validation and a working workflow dispatcher.
...
- [ ] Document density-compensation/cancellation tests for internal-wave-like dipoles.
...
- [ ] Define time series sampling, Fourier normalization, windowing, detrending, and one-sided PSD conventions.
- [ ] Implement time-to-frequency conversion with Parseval tests.
...
- [ ] Encode HUST facility parameters only when traceable to an authoritative specification.
...
- [ ] Submarine landslide: coupled solid-water rapid mass migration with conservation checks.
