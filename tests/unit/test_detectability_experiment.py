"""Determinism and coverage-safety tests for P1-E002."""
...
from run_p1_detectability_foundation import run  # noqa: E402
...
class TestDetectabilityExperiment(unittest.TestCase):
    ...
    def test_result_is_deterministic_and_complete(self) -> None:
        ...
        self.assertTrue(
            all(len(instruments) == 3 for instruments in self.result["matrix"].values())
        )

    def test_gradient_curves_are_explicitly_excluded(self) -> None:
        excluded = self.result["excluded_instrument_curves"]
        self.assertEqual(len(excluded), 2)
        self.assertTrue(all("gravity_gradient" in reason for reason in excluded.values()))
