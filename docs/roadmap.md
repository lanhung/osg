...
- [ ] Implement rapid source-parameter inversion for magnitude, region, and rupture segment. (A strict discrete template-library inversion reports winning library Mw/segment, null-model improvement and second-best ambiguity under an independent-noise reference, with versioned templates, noise-scale provenance and explicit decision time since origin. Validated PEGS templates, continuous/source-uncertainty inference and real covariance remain.)
